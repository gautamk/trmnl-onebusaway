...
1. The worker will make multiple calls to the OneBusAway arrivals and departures API  https://developer.onebusaway.org/api/where/methods/arrivals-and-departures-for-stop
2. There will be a list of transit stops, the worker should iterate over the list, and call the OneBusAway API to fetch arrivals and departures for each transit stop.
3. The list of transit stops needs to be configurable. Each transit stop will have an associated nickname. 
4. The data fetched from the OneBusAway API needs to be transformed by selecting some fields from the API responses. 
5. The fields to select should be configurable. 
6. The selected fields should be collected into a JSON payload
7. The JSON payload should be sent to a Webhook URL
8. The webhook URL should be configurable.
9. All configurations should be in code. 
10. If possible Avoid using databases
11. If possible, prefer use of durable objects for caching data
12. Create this worker in a directory called `oba-webhook`
</user_prompt>